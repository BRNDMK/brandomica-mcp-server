...
import { z } from "zod";
const API_BASE = process.env.BRANDOMICA_API_URL || "https://www.brandomica.com";
const server = new McpServer({
    name: "brandomica-mcp-server",
    version: "1.0.3",
}, {
    instructions: "Brandomica Lab — safety-first brand name verification. " +
...
async function fetchApi(endpoint, name, extra) {
...
async function fetchApiPost(endpoint, body) {
...
function formatDomains(data) {
    const lines = ["## Domain Availability"];
    for (const d of data.results) {
        ...
        if (d.available && d.purchasePrice != null && d.renewalPrice != null) {
            ...
            const upfrontNote = minYears > 1 ? `, due today $${dueToday} (${minYears}y minimum)` : "";
...
function formatSocial(data) {
    const lines = ["## Social Handle Availability"];
...
function formatTrademarks(data) {
    const lines = ["## Trademark Search"];
...
function formatAppStores(data) {
    const lines = ["## App Store Search"];
...
function formatSaas(data) {
    const lines = ["## Package Registries & SaaS"];
    for (const s of data.results) {
        const status = s.available === true
            ? "Available"
            : s.available === false
...
function formatGoogle(data) {
    const lines = ["## Web Presence (Google Search)"];
    for (const g of data.results) {
        const status = g.found === true
            ...
            : g.found === false
                ...
                : "Unknown (check manually)";
        const topHits = g.found === true && g.topResults?.length
...
const brandNameInput = {
    brand_name: z
        ...
        .min(1)
        .max(63)
        .regex(/^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/, "Lowercase letters, numbers, and hyphens only (no leading/trailing hyphens)")
...
server.registerTool("brandomica_check_all", {
    ...
    const data = (await fetchApi("check-all", brand_name, extra));
    return {
        content: [{ type: "text", text: JSON.stringify(data) }],
    };
});
server.registerTool("brandomica_assess_safety", {
    ...
    const data = (await fetchApi("check-all", brand_name, extra));
    return {
        content: [{ type: "text", text: JSON.stringify(data.safety) }],
    };
});
server.registerTool("brandomica_filing_readiness", {
    ...
    const extra = mode && mode !== "full" ? { mode } : undefined;
    const data = (await fetchApi("filing-readiness", brand_name, extra));
    return {
        content: [{ type: "text", text: JSON.stringify(data) }],
    };
});
server.registerTool("brandomica_compare_brands", {
    ...
    inputSchema: z.object({
        brand_names: z
            .array(z.string().min(1).max(63).regex(/^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/, "Lowercase letters, numbers, and hyphens only"))
            .min(2)
            .max(5)
    ...
    const data = (await fetchApiPost("compare-brands", { names: brand_names }));
    return {
        content: [{ type: "text", text: JSON.stringify(data) }],
    };
});
server.registerTool("brandomica_brand_report", {
...
server.registerTool("brandomica_check_domains", {
    title: "Domain Availability",
    description: "Check domain availability across 6 TLDs (.com, .io, .co, .app, .dev, .ai) with purchase and renewal pricing.",
    inputSchema: z.object(brandNameInput).strict(),
    annotations: toolAnnotations,
}, async ({ brand_name }) => {
    const data = (await fetchApi("check-domains", brand_name));
    return {
        content: [{ type: "text", text: formatDomains(data) }],
    };
});
server.registerTool("brandomica_check_social", {
    ...
    const data = (await fetchApi("check-social", brand_name));
    return {
        content: [{ type: "text", text: formatSocial(data) }],
    };
});
server.registerTool("brandomica_check_trademarks", {
    ...
    const data = (await fetchApi("check-trademarks", brand_name));
    return {
        content: [{ type: "text", text: formatTrademarks(data) }],
    };
});
server.registerTool("brandomica_check_appstores", {
    title: "App Store Search",
    description: "Search iOS App Store and Google Play for apps matching the brand name.",
    inputSchema: z.object(brandNameInput).strict(),
    annotations: toolAnnotations,
}, async ({ brand_name }) => {
    const data = (await fetchApi("check-appstores", brand_name));
    return {
        content: [{ type: "text", text: formatAppStores(data) }],
    };
});
server.registerTool("brandomica_check_saas", {
    ...
    const data = (await fetchApi("check-saas", brand_name));
    return {
        content: [{ type: "text", text: formatSaas(data) }],
    };
});
server.registerTool("brandomica_check_google", {
    ...
    const data = (await fetchApi("check-google", brand_name));
    return {
        content: [{ type: "text", text: formatGoogle(data) }],
    };
});
server.registerTool("brandomica_batch_check", {
    title: "Batch Brand Check",
    description: "Check 2-10 brand names in a single call. Runs checks concurrently and returns results sorted by score descending. Each result includes availability score and safety assessment.",
    inputSchema: z.object({
        brand_names: z
            .array(z.string().min(1).max(63).regex(/^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/, "Lowercase letters, numbers, and hyphens only"))
            .min(2)
